...
export function SettingsClient() {
  ...
  const handleConnectGemini = async () => {
    ...
    try {
      ...
      setGeminiAlert(null);
      setGeminiError(null);
      const { url } = await GeminiService.startOAuth();
      window.location.href = url;
    } catch (error) {
      ...
      setGeminiAlert({
        ...
        text:
          error instanceof Error
            ? error.message
            : 'No pudimos abrir la autorización de Google Gemini. Verificá tu configuración e intentá nuevamente.',
      });
    } finally {
        ...
        {isInvitedProfessional ? (
          ...
          <>
            <div className="grid gap-3 rounded-2xl border border-emerald-400/30 bg-emerald-500/5 p-4 text-sm text-emerald-100">
              <div>
                <p className="text-base font-semibold text-emerald-100">Gemini conectado</p>
                <p className="text-emerald-200/80">
                  Generando insights con la cuenta{' '}
                  <span className="font-semibold">{geminiStatus.email ?? 'autorizada en Google'}</span>.
                </p>
              </div>
              <div className="grid gap-2 text-xs text-emerald-200/70 sm:grid-cols-2">
                ...
                {geminiStatus.updatedAt && <p>Última sincronización: {formatDateTime(geminiStatus.updatedAt)}</p>}
                {geminiStatus.expiresAt && <p>Renovación automática: {formatDateTime(geminiStatus.expiresAt)}</p>}
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-3">
              <button
                ...
                className="rounded-full border border-emerald-400/40 px-6 py-2 text-sm font-semibold text-emerald-100 transition hover:bg-emerald-400/10 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {geminiActionLoading ? 'Desconectando...' : 'Desconectar Google Gemini'}
              </button>
              <button
                type="button"
          ...
          <div className="flex flex-col gap-4 rounded-2xl border border-white/15 bg-slate-900/60 p-4 text-sm text-slate-200">
            <p>
              Vinculá tu cuenta para que Dentalist genere reportes, propuestas para WhatsApp e Instagram y tableros con datos
              reales.
            </p>
            <div className="flex flex-wrap items-center gap-3">
              <button
                ...
                className="rounded-full bg-cyan-500 px-6 py-2 text-sm font-semibold text-slate-950 shadow-lg shadow-cyan-500/30 transition hover:bg-cyan-400 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {geminiActionLoading ? 'Abriendo Google...' : 'Conectar con Google Gemini'}
              </button>
              <p className="text-xs text-slate-400">
                Podés reutilizar el mismo cliente OAuth que usás para Google Calendar. Solo asegurate de incluir el callback de
                Gemini en los redirects autorizados.
              </p>
            </div>
          </div>
        )}
