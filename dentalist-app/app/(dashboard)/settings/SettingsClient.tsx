...
import { GeminiConnectionStatus, OrthodonticPlan, ProfessionalKeyStatus, ProfessionalProfile } from '@/types';
...
export function SettingsClient() {
  ...
  const [geminiAlert, setGeminiAlert] = useState<{ type: 'success' | 'error'; text: string } | null>(null);
  const [geminiActionLoading, setGeminiActionLoading] = useState(false);
  const [geminiRefreshIndex, setGeminiRefreshIndex] = useState(0);
  ...
  const hasAiCapability =
    isOwnerProfessional && planSupportsCapability(user?.subscriptionPlan ?? null, 'aiInsights');
  ...
  useEffect(() => {
    ...
    if (!hasAiCapability && !isInvitedProfessional) {
    ...
    const loadGeminiStatus = async () => {
      try {
        ...
        setGeminiLabelInput(status?.label ?? '');
        setGeminiKeyInput('');
      } catch (error) {
        console.error('Error al consultar el estado de Gemini', error);
        if (active) {
          setGeminiStatus(null);
          setGeminiError(
            error instanceof Error
              ? error.message
              : 'No pudimos consultar el estado de Google Gemini. Intentá nuevamente.',
          );
        }
      } finally {
        if (active) {
  ...
  }, [hasAiCapability, isInvitedProfessional, geminiRefreshIndex]);
  ...
  useEffect(() => {
    ...
    const geminiStatusParam = searchParams.get('gemini');
    ...
    if (geminiStatusParam === 'connected') {
      setGeminiAlert({ type: 'success', text: 'Google Gemini se vinculó correctamente.' });
      setGeminiRefreshIndex((value) => value + 1);
    } else if (geminiStatusParam === 'error') {
      setGeminiAlert({
        type: 'error',
        text: messageParam ?? 'No pudimos vincular Google Gemini. Intentá nuevamente.',
      });
    }
  }, [searchParams]);
  ...
  const handleConnectGemini = async () => {
    if (!hasAiCapability) {
      setGeminiAlert({
        type: 'error',
        text: 'Tu plan actual no incluye automatizaciones con Google Gemini.',
      });
      return;
    }
    ...
    if (!trimmedKey) {
      setGeminiAlert({
        type: 'error',
        text: 'Ingresá la API key generada en Google AI Studio para continuar.',
      });
      return;
    }

    try {
      setGeminiActionLoading(true);
      setGeminiAlert(null);
      setGeminiError(null);
      await GeminiService.saveApiKey({
        apiKey: trimmedKey,
        label: geminiLabelInput.trim().length > 0 ? geminiLabelInput.trim() : undefined,
      });
      setGeminiAlert({ type: 'success', text: 'Guardamos la API key de Google AI Studio correctamente.' });
      setGeminiKeyInput('');
      setGeminiRefreshIndex((value) => value + 1);
    } catch (error) {
      console.error('Error al iniciar la conexión con Google Gemini', error);
      setGeminiAlert({
        type: 'error',
        text:
          error instanceof Error
            ? error.message
            : 'No pudimos guardar la API key. Verificá el valor ingresado e intentá nuevamente.',
      });
    } finally {
      setGeminiActionLoading(false);
    }
  };
  ...
  const handleDisconnectGemini = async () => {
    ...
    try {
      ...
      setGeminiAlert({ type: 'success', text: 'Se desconectó Google Gemini correctamente.' });
      setGeminiStatus((previous) =>
        previous
          ? {
              ...previous,
              connected: false,
      ...
      console.error('Error al desconectar Google Gemini', error);
      setGeminiAlert({
        type: 'error',
        text:
          error instanceof Error
            ? error.message
            : 'No pudimos desconectar Google Gemini. Intentá nuevamente más tarde.',
      });
    } finally {
      setGeminiActionLoading(false);
    }
  ...
  const handleRefreshGeminiStatus = () => {
    setGeminiRefreshIndex((value) => value + 1);
  };
  ...
  const handleGeminiFormSubmit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    handleConnectGemini();
  };
  ...
  return (
    <section className="space-y-8">
      ...
      <div className="grid gap-6 rounded-3xl border border-white/10 bg-white/5 p-8 shadow-lg shadow-cyan-500/10">
        <div>
          <h2 className="text-lg font-semibold text-white">Google Gemini &amp; automatizaciones IA</h2>
          <p className="text-xs text-slate-400">
            Conectá tu cuenta de Google para que Dentalist genere insights y campañas con Gemini en base a tus métricas reales.
          </p>
        </div>
        ...
        {isInvitedProfessional ? (
          <div className="rounded-2xl border border-amber-400/40 bg-amber-500/10 p-4 text-sm text-amber-100">
            El administrador del consultorio gestiona la conexión con Gemini. Pedile que active la integración para que puedas
            consultar los insights en tu cuenta.
          </div>
        ) : !hasAiCapability ? (
          <div className="rounded-2xl border border-amber-400/40 bg-amber-500/10 p-4 text-sm text-amber-100">
            Tu plan actual no incluye automatizaciones con IA. Mejorá al plan{' '}
            <Link href="/billing" className="font-semibold underline-offset-4 hover:underline">
              Pro
            </Link>{' '}
            para conectar Google Gemini y recibir campañas generadas automáticamente.
          </div>
        ) : geminiLoading ? (
          <p className="text-sm text-slate-300">Consultando el estado de Google Gemini...</p>
        ) : (
          <div className="grid gap-4 rounded-2xl border border-white/15 bg-slate-900/60 p-4 text-sm text-slate-200">
            {geminiStatus?.connected ? (
              <div className="grid gap-3 rounded-2xl border border-emerald-400/30 bg-emerald-500/5 p-4 text-sm text-emerald-100">
                <p className="text-base font-semibold text-emerald-100">API key almacenada</p>
                <div className="grid gap-2 text-xs text-emerald-200/80 sm:grid-cols-2">
                  <p>Modelo: {geminiStatus.model ?? 'models/gemini-1.5-flash-latest'}</p>
                  {geminiStatus.label && <p>Etiqueta: {geminiStatus.label}</p>}
                  {geminiStatus.updatedAt && <p>Actualizada: {formatDateTime(geminiStatus.updatedAt)}</p>}
                  {geminiStatus.lastUsedAt && <p>Último uso: {formatDateTime(geminiStatus.lastUsedAt)}</p>}
                </div>
              </div>
            ) : (
              <p>
                Guardá tu API key de Google AI Studio para que Dentalist genere reportes, insights y campañas personalizadas
                con los datos reales de tu clínica.
              </p>
            )}

            <form onSubmit={handleGeminiFormSubmit} className="grid gap-3">
              <label className="text-xs font-semibold uppercase tracking-wide text-slate-400" htmlFor="gemini-api-key">
              ...
              <input
                ...
                placeholder="AIza..."
                className="w-full rounded-2xl border border-white/20 bg-black/40 px-4 py-2 text-sm text-white placeholder:text-slate-500 focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-400/40"
              />

              <label className="text-xs font-semibold uppercase tracking-wide text-slate-400" htmlFor="gemini-label">
                Etiqueta (opcional)
              </label>
              <input
                id="gemini-label"
                type="text"
                value={geminiLabelInput}
                onChange={(event) => setGeminiLabelInput(event.target.value)}
                placeholder="Cuenta principal, Clínica Central, etc."
                className="w-full rounded-2xl border border-white/20 bg-black/40 px-4 py-2 text-sm text-white placeholder:text-slate-500 focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-400/40"
              />
              ...
              <div className="flex flex-wrap items-center gap-3 pt-1">
                ...
                <button
                  type="button"
                  onClick={handleRefreshGeminiStatus}
                  disabled={geminiActionLoading || geminiLoading}
                  className="rounded-full border border-white/20 px-6 py-2 text-sm font-semibold text-slate-200 transition hover:border-cyan-300/60 hover:text-white disabled:cursor-not-allowed disabled:opacity-60"
                >
                  Actualizar estado
                </button>
                {geminiStatus?.connected && (
                  <button
                    ...
                    disabled={geminiActionLoading}
                    className="rounded-full border border-rose-400/40 px-6 py-2 text-sm font-semibold text-rose-200 transition hover:bg-rose-400/10 disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    {geminiActionLoading ? 'Procesando...' : 'Eliminar API key'}
                  </button>
                )}
              </div>

              <p className="text-xs text-slate-400">
                Generá la clave desde{' '}
                <a
                ...
                y pegala aquí. Tus datos se cifran automáticamente con tu clave maestra antes de guardarse en Supabase.
              </p>
            </form>
          </div>
        )}
