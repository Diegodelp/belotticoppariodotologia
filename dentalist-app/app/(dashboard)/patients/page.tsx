...
export default function PatientsPage() {
  ...
  useEffect(() => {
    ...
    const fetchClinics = async () => {
      try {
        ...
        console.error('Error al cargar consultorios', clinicError);
        if (active) {
          setClinics([]);
        }
      } finally {
  ...
  const hasUnassignedPatients = useMemo(() => patients.some((patient) => !patient.clinicName), [patients]);

  const filteredPatients = useMemo(() => {
    const normalizedSearch = search.trim().toLowerCase();
    const selectedClinicName =
      clinicFilter !== 'all' && clinicFilter !== '__none__'
        ? clinics.find((clinic) => clinic.id === clinicFilter)?.name?.toLowerCase() ?? null
        : null;
    return patients.filter((patient) => {
      ...
      const matchesClinic = !enableClinicFilter
        ? true
        : clinicFilter === 'all'
        ? true
        : clinicFilter === '__none__'
        ? !patient.clinicName
        : selectedClinicName
        ? (patient.clinicName ?? '').toLowerCase() === selectedClinicName
        : false;
      return matchesSearch && matchesStatus && matchesClinic;
    });
  }, [patients, search, status, clinicFilter, clinics, enableClinicFilter]);
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-3">
            {enableClinicFilter && (
              <select
                ...
                className="w-full rounded-full border border-white/10 bg-slate-950/60 px-4 py-3 text-sm text-white transition focus:border-cyan-300 focus:outline-none focus:ring-2 focus:ring-cyan-300/40 md:w-64"
              >
