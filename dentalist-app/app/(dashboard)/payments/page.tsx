...
export default function PaymentsPage() {
    ...
    const list = payments.filter((payment) => {
      ...
      if (enableClinicFilter) {
        if (clinicFilter === '__none__') {
          ...
          if ((payment.patient?.clinicName ?? '').toLowerCase() !== selectedClinicName) {
            return false;
          }
        }
      }
  ...
  }, [fromDate, payments, status, toDate, clinicFilter, clinics, enableClinicFilter]);
          ...
          <div className="flex flex-col gap-3 text-xs text-slate-200 sm:flex-row sm:items-center">
            {enableClinicFilter && (
              <select
                ...
                className="w-full rounded-full border border-white/10 bg-slate-950/60 px-3 py-2 text-sm text-white focus:border-cyan-300 focus:outline-none focus:ring-2 focus:ring-cyan-300/30 sm:w-56"
              >
