...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const calendarCredentials = await getProfessionalGoogleCredentials(payload.userId);

    const refreshToken =
      tokens.refresh_token ?? existing?.refreshToken ?? calendarCredentials?.refreshToken;
    ...
    const expiryDate =
      typeof tokens.expiry_date === 'number'
        ...
        : existing?.expiryDate ?? calendarCredentials?.expiryDate ?? null;

    const tokenSet: OAuthTokenSet = {
      accessToken,
      refreshToken,
      scope: tokens.scope ?? existing?.scope ?? calendarCredentials?.scope ?? null,
      tokenType: tokens.token_type ?? existing?.tokenType ?? calendarCredentials?.tokenType ?? null,
      expiryDate,
    };
    ...
    const googleUserId =
      ...
      calendarCredentials?.googleUserId ??
      null;
    const email =
      profile?.email ?? idTokenClaims?.email ?? existing?.email ?? calendarCredentials?.email ?? null;
