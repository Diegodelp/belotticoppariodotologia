...
function sanitizeInput(body: unknown): ClinicalHistoryInput {
  ...
  const signatureClarification = sanitizeString(isBodyRecord ? body.signatureClarification : undefined);
  const signatureDataUrl =
    isBodyRecord && typeof body.signatureDataUrl === 'string' && body.signatureDataUrl.trim().length > 0
      ? body.signatureDataUrl
      : undefined;
  const removeSignature = isBodyRecord && body.removeSignature === true;
