...
import {
  ...
  listPayments,
  listTreatments,
  markProfessionalGeminiKeyUsage,
...
export async function GET(request: NextRequest) {
  ...
  if (!credentials) {
    return NextResponse.json(
      {
        error:
          'Guardá tu API key de Google AI Studio desde Configuración > Google & IA para generar insights con tus datos reales.',
      },
      { status: 412 },
    );
  }
...
Redactá en español rioplatense, sé concreto y basate solo en los datos recibidos. JSON de entrada:\n${JSON.stringify(
    ...
    const { text, model } = await generateGeminiContent(credentials.apiKey, prompt, {
      temperature: 0.35,
    });

    markProfessionalGeminiKeyUsage(user.id).catch((error) => {
      console.warn('No se pudo registrar el uso de Gemini', error);
    });
