...
export function getClinicLimit(plan: SubscriptionPlan | null | undefined): number | null {
  return getPlanDefinition(plan).capabilities.clinicLimit;
}
...
export function planSupportsCapability(
  ...
  switch (capability) {
    ...
    case 'clinicLimit': {
      const starterLimit = PLAN_DEFINITIONS.starter.capabilities.clinicLimit;
      if (starterLimit === null) {
        return true;
      }
      const planLimit = definition.capabilities.clinicLimit;
      return planLimit === null || (planLimit ?? 0) > (starterLimit ?? 0);
    }
    case 'storage':
