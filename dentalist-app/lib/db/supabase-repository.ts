...
const GEMINI_CREDENTIALS_TABLE =
  process.env.SUPABASE_TABLE_GEMINI_CREDENTIALS ?? 'professional_gemini_credentials';
...
async function uploadClinicalHistorySignature(
  client: ReturnType<typeof getClient>,
  professionalId: string,
  patientId: string,
  ...
  const extension = mimeType.includes('png')
    ...
    : mimeType.includes('jpeg') || mimeType.includes('jpg')
      ? 'jpg'
      : 'png';
  const storagePath = `${professionalId}/patients/${patientId}/clinical-history/signature-${Date.now()}.${extension}`;
...
async function deleteClinicalHistorySignature(
  client: ReturnType<typeof getClient>,
  ...
  try {
    ...
    if (error) {
      console.warn('No se pudo eliminar la firma de la historia clínica del storage', error);
    }
  } catch (error) {
    console.warn('No se pudo eliminar la firma de la historia clínica del storage', error);
  }
...
async function uploadTreatmentConsentAssets(
  client: ReturnType<typeof getClient>,
  professionalId: string,
  patientId: string,
  treatmentId: string,
...
): Promise<{ pdfPath: string; pdfName: string; signaturePath: string }> {
  await ensureBucketExists(client, DOCUMENTS_BUCKET);

  const pdfMime = payload.pdf.mimeType?.startsWith('application/') ? payload.pdf.mimeType : 'application/pdf';
  const pdfExtension = resolveFileExtension(payload.pdf.fileName, pdfMime);
  const sanitizedPdfBase = sanitizeFileName(payload.pdf.fileName) ?? `consentimiento-${treatmentId}`;
  const pdfFileName = sanitizedPdfBase.toLowerCase().endsWith(`.${pdfExtension}`)
    ? sanitizedPdfBase
    : `${sanitizedPdfBase}.${pdfExtension}`;
  const pdfPath = `${professionalId}/patients/${patientId}/treatments/${treatmentId}/${pdfFileName}`;

  const { error: pdfError } = await client.storage
    .from(DOCUMENTS_BUCKET)
    .upload(pdfPath, payload.pdf.buffer, {
      contentType: pdfMime,
      upsert: true,
    });
  ...
  const signatureMime = payload.signature.mimeType?.startsWith('image/')
    ? payload.signature.mimeType
    : 'image/png';
  const signatureExtension = resolveFileExtension(null, signatureMime);
  const signaturePath = `${professionalId}/patients/${patientId}/treatments/${treatmentId}/consent-signature.${signatureExtension}`;

  try {
    const { error: signatureError } = await client.storage
      .from(DOCUMENTS_BUCKET)
      .upload(signaturePath, payload.signature.buffer, {
    ...
    await deleteTreatmentConsentAssets(client, [pdfPath]);
    throw error;
  }
...
async function deleteTreatmentConsentAssets(
  client: ReturnType<typeof getClient>,
  paths: Array<string | null | undefined>,
) {
  const filtered = paths.filter((path): path is string => typeof path === 'string' && path.length > 0);
  if (!filtered.length) {
    return;
  }
  ...
  try {
    await ensureBucketExists(client, DOCUMENTS_BUCKET);
    const { error } = await client.storage.from(DOCUMENTS_BUCKET).remove(filtered);
    if (error) {
      console.warn('No se pudieron eliminar los archivos de consentimiento del storage', error);
    }
  } catch (error) {
    console.warn('No se pudieron eliminar los archivos de consentimiento del storage', error);
  }
...
type AppClinicalHistoryRow = {
  ...
  signature_clarification: string | null;
...
function mapAppointment(record: AppAppointmentRow): Appointment {
  ...
  return {
    ...
    clinicId: record.clinic_id ?? null,
    checkedInAt: record.checked_in_at ? new Date(record.checked_in_at).toISOString() : null,
    checkedInByName: record.checked_in_by_name ?? null,
    calledAt: record.called_at ? new Date(record.called_at).toISOString() : null,
    calledByName: record.called_by_name ?? null,
    calledBox: record.called_box ?? null,
  };
}

function mapTreatment(
  record: AppTreatmentRow,
  assets?: { consentPdfUrl?: string | null; consentSignatureUrl?: string | null },
): Treatment {
  const rawDate =
  ...
  const consentStatus = (record.consent_status ?? undefined) as Treatment['consent'] extends infer T
    ? T extends { status?: infer S }
      ? S | undefined
      : undefined
    : undefined;

  const consent: Treatment['consent'] =
    record.consent_template_path ||
    record.consent_patient_name ||
    record.consent_signature_path ||
    record.consent_signed_at
      ? {
          pdfUrl: assets?.consentPdfUrl ?? null,
          pdfName: record.consent_template_name ?? null,
          signedAt: record.consent_signed_at ?? null,
          patientName: record.consent_patient_name ?? null,
          patientSignatureUrl: assets?.consentSignatureUrl ?? null,
          status: consentStatus ?? (record.consent_template_path ? 'signed' : undefined),
        }
      : null;
...
function mapClinicalHistory(row: AppClinicalHistoryRow): ClinicalHistory {
  ...
  return {
    ...
    stages,
    signatureClarification: getOptionalString(row.signature_clarification) ?? null,
    signatureUrl: null,
    signatureUpdatedAt: row.signature_uploaded_at ?? null,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
  };
}
...
function mapPatientOrthodonticPlan(row: AppPatientOrthodonticPlanRow): PatientOrthodonticPlan {
  return {
    ...
    treatmentGoal: row.treatment_goal ?? null,
    appliance: row.appliance ?? null,
    controlFrequency: row.control_frequency ?? null,
    estimatedDuration: row.estimated_duration ?? null,
    planNotes: row.plan_notes ?? null,
  };
}
...
export async function assignOrthodonticPlanToPatient(
  ...
  const { data: existingAssignment, error: existingAssignmentError } = await client
    ...
    .select('treatment_goal, appliance, control_frequency, estimated_duration, plan_notes')
    .eq('professional_id', professionalId)
    ...
    .upsert(
      {
        ...
        treatment_goal: existingAssignment?.treatment_goal ?? null,
        appliance: existingAssignment?.appliance ?? null,
        control_frequency: existingAssignment?.control_frequency ?? null,
        estimated_duration: existingAssignment?.estimated_duration ?? null,
        plan_notes: existingAssignment?.plan_notes ?? null,
      },
...
export async function updatePatientOrthodonticPlanDetails(
  professionalId: string,
  patientId: string,
  details: UpdateOrthodonticPlanDetailsInput,
): Promise<PatientOrthodonticPlan> {
  const client = getClient();
  ...
  const treatmentGoal = sanitize(details.treatmentGoal);
  if (treatmentGoal !== undefined) {
    updates.treatment_goal = treatmentGoal;
  }
  const appliance = sanitize(details.appliance);
  if (appliance !== undefined) {
    updates.appliance = appliance;
  }
  const controlFrequency = sanitize(details.controlFrequency);
  if (controlFrequency !== undefined) {
    updates.control_frequency = controlFrequency;
  }
  const estimatedDuration = sanitize(details.estimatedDuration);
  if (estimatedDuration !== undefined) {
    updates.estimated_duration = estimatedDuration;
  }
  const planNotes = sanitize(details.planNotes);
  if (planNotes !== undefined) {
    updates.plan_notes = planNotes;
  }

  if (Object.keys(updates).length === 0) {
    const existing = await getPatientOrthodonticPlan(professionalId, patientId);
    if (!existing) {
      throw new Error('El paciente no tiene un plan asignado');
    }
    return existing;
  }
...
export interface ProfessionalGeminiCredentials {
  professionalId: string;
  apiKey: string;
  label: string | null;
  updatedAt?: string | null;
...
export async function getProfessionalGeminiCredentials(
  professionalId: string,
): Promise<ProfessionalGeminiCredentials | null> {
  const client = getClient();
  const { data, error } = await client
    .from(GEMINI_CREDENTIALS_TABLE)
    .select('*')
    .eq('professional_id', professionalId)
    .maybeSingle();
  ...
  const record = data as AppGeminiCredentialRow;
  const apiKey = await decryptSensitivePayload<string>(professionalId, {
    ciphertext: record.api_key_ciphertext,
    iv: record.api_key_iv,
    version: record.api_key_version,
  });
...
export async function upsertProfessionalGeminiCredentials(
  professionalId: string,
  credentials: { apiKey: string; label?: string | null },
) {
  const client = getClient();
  const encrypted = await encryptSensitivePayload(professionalId, credentials.apiKey);
  const { error } = await client.from(GEMINI_CREDENTIALS_TABLE).upsert(
    {
      professional_id: professionalId,
      api_key_ciphertext: encrypted.ciphertext,
      api_key_iv: encrypted.iv,
      api_key_version: encrypted.version,
...
export async function deleteProfessionalGeminiCredentials(professionalId: string) {
  const client = getClient();
  const { error } = await client
    .from(GEMINI_CREDENTIALS_TABLE)
    .delete()
...
export async function markProfessionalGeminiKeyUsage(professionalId: string) {
  const client = getClient();
  const { error } = await client
    .from(GEMINI_CREDENTIALS_TABLE)
...
export async function markAppointmentCheckedIn(
  professionalId: string,
  appointmentId: string,
  payload: { checkedInAt: string; checkedInByName?: string | null },
) {
  const client = getClient();
  const { data, error } = await client
    .from(APPOINTMENTS_TABLE)
    .update({
      ...
      checked_in_by_name: payload.checkedInByName ?? null,
    })
    .eq('professional_id', professionalId)
    .eq('id', appointmentId)
    .select('*')
...
export async function markAppointmentCalled(
  professionalId: string,
  appointmentId: string,
  payload: { calledAt: string; calledBox: string; calledByName?: string | null },
) {
  const client = getClient();
  const { data, error } = await client
    .from(APPOINTMENTS_TABLE)
    .update({
      ...
      called_box: payload.calledBox,
      called_by_name: payload.calledByName ?? null,
    })
    .eq('professional_id', professionalId)
    .eq('id', appointmentId)
    .select('*')
...
export async function getLatestCalledAppointment(
  professionalId: string,
...
export async function listTreatments(
  ...
  const signedAssets = await Promise.all(
    rows.map(async (row) => ({
      pdfUrl: row.consent_template_path
        ? await createSignedUrl(client, DOCUMENTS_BUCKET, row.consent_template_path, 900)
        : null,
      signatureUrl: row.consent_signature_path
        ? await createSignedUrl(client, DOCUMENTS_BUCKET, row.consent_signature_path, 900)
        : null,
    })),
  );
  return rows.map((row, index) =>
    mapTreatment(row, {
      consentPdfUrl: signedAssets[index]?.pdfUrl ?? null,
      consentSignatureUrl: signedAssets[index]?.signatureUrl ?? null,
    }),
  );
}
...
export async function createTreatment(
  ...
  const { data, error } = await client
    ...
    .insert({
      ...
      consent_patient_name: treatment.consent.patientName,
      consent_status: 'signed',
      consent_signed_at: insertedAt,
      consent_template_name: sanitizeFileName(treatment.consent.pdf.fileName) ?? 'consentimiento.pdf',
    })
    .select('*')
    .single();
  if (error) throw error;
  const row = data as AppTreatmentRow;

  const { pdfPath, pdfName, signaturePath } = await uploadTreatmentConsentAssets(
    client,
    professionalId,
    treatment.patientId,
    row.id,
    {
      pdf: treatment.consent.pdf,
      signature: treatment.consent.signature,
    },
  );
  ...
  const { data: updatedRow, error: updateError } = await client
    .from(TREATMENTS_TABLE)
    .update({
      consent_template_path: pdfPath,
      consent_template_name: pdfName,
  ...
  if (updateError) throw updateError;
  const updated = (updatedRow ?? row) as AppTreatmentRow;

  const [pdfUrl, signatureUrl] = await Promise.all([
    createSignedUrl(client, DOCUMENTS_BUCKET, pdfPath, 900),
    createSignedUrl(client, DOCUMENTS_BUCKET, signaturePath, 900),
  ]);

  return mapTreatment(updated, {
...
export async function updateTreatment(
  ...
  const existing = existingRow as AppTreatmentRow;

  let consentTemplatePath = existing.consent_template_path ?? null;
  let consentTemplateName = existing.consent_template_name ?? null;
  let consentSignaturePath = existing.consent_signature_path ?? null;
  let consentSignedAt = existing.consent_signed_at ?? null;
  let consentStatus = existing.consent_status ?? (existing.consent_template_path ? 'signed' : null);
  let consentPatientName = updates.consent?.patientName ?? existing.consent_patient_name ?? null;

  if (updates.consent?.replace) {
    if (!updates.consent.pdf || !updates.consent.signature) {
      throw new Error('Debés adjuntar el consentimiento informado y la firma del paciente.');
    }

    await deleteTreatmentConsentAssets(client, [consentTemplatePath, consentSignaturePath]);

    const uploaded = await uploadTreatmentConsentAssets(
      client,
      professionalId,
      existing.patient_id,
      treatmentId,
      {
        pdf: updates.consent.pdf,
        signature: updates.consent.signature,
      },
    );

    consentTemplatePath = uploaded.pdfPath;
    consentTemplateName = uploaded.pdfName;
    consentSignaturePath = uploaded.signaturePath;
    consentSignedAt = new Date().toISOString();
    consentStatus = 'signed';
  } else if (updates.consent?.patientName) {
    consentPatientName = updates.consent.patientName;
  }
  ...
  const { data: updatedRow, error: updateError } = await client
    ...
    .update({
      ...
      consent_template_path: consentTemplatePath,
      consent_template_name: consentTemplateName,
      consent_signature_path: consentSignaturePath,
      consent_signed_at: consentSignedAt,
      consent_status: consentStatus,
      consent_patient_name: consentPatientName,
    })
    .eq('professional_id', professionalId)
    .eq('id', treatmentId)
    .select('*')
  ...
  const updated = updatedRow as AppTreatmentRow;
  const [pdfUrl, signatureUrl] = await Promise.all([
    consentTemplatePath ? createSignedUrl(client, DOCUMENTS_BUCKET, consentTemplatePath, 900) : Promise.resolve(null),
    consentSignaturePath ? createSignedUrl(client, DOCUMENTS_BUCKET, consentSignaturePath, 900) : Promise.resolve(null),
  ]);
...
export async function deleteTreatment(professionalId: string, treatmentId: string): Promise<void> {
  ...
  const { data: existingRow, error: fetchError } = await client
    .from(TREATMENTS_TABLE)
    .select('consent_template_path, consent_signature_path')
    .eq('professional_id', professionalId)
    .eq('id', treatmentId)
    .maybeSingle();
  ...
  if (existingRow) {
    await deleteTreatmentConsentAssets(client, [existingRow.consent_template_path, existingRow.consent_signature_path]);
  }
}
...
export async function getClinicalHistory(
  ...
  const row = data as AppClinicalHistoryRow;
  const history = mapClinicalHistory(row);

  if (row.signature_path) {
    history.signatureUrl = await createSignedUrl(client, DOCUMENTS_BUCKET, row.signature_path, 900);
  }
...
export async function upsertClinicalHistory(
  ...
  const signatureClarificationInput =
    typeof input.signatureClarification !== 'undefined'
      ? normalizeString(input.signatureClarification)
      : undefined;
  const signatureDataUrl = typeof input.signatureDataUrl === 'string' ? input.signatureDataUrl.trim() : '';
  const shouldRemoveSignature = input.removeSignature === true && !signatureDataUrl;
  ...
  const existingSignaturePath = (existing as { signature_path?: string | null } | null)?.signature_path ?? null;
  let signaturePathToPersist: string | null | undefined;
  let signatureUploadedAtValue: string | null | undefined;

  if (signatureDataUrl) {
    const file = parseSignatureDataUrl(signatureDataUrl);
    const storagePath = await uploadClinicalHistorySignature(client, professionalId, patientId, file);
    signaturePathToPersist = storagePath;
    signatureUploadedAtValue = new Date().toISOString();
    if (existingSignaturePath && existingSignaturePath !== storagePath) {
      await deleteClinicalHistorySignature(client, existingSignaturePath);
    }
  } else if (shouldRemoveSignature) {
    if (existingSignaturePath) {
      await deleteClinicalHistorySignature(client, existingSignaturePath);
    }
    signaturePathToPersist = null;
    signatureUploadedAtValue = null;
  }
  ...
  if (existing) {
    ...
    if (typeof signatureClarificationInput !== 'undefined') {
      updatePayload.signature_clarification = signatureClarificationInput;
    }

    if (signaturePathToPersist !== undefined) {
      updatePayload.signature_path = signaturePathToPersist;
      updatePayload.signature_uploaded_at =
        signaturePathToPersist && signatureUploadedAtValue === undefined
    ...
    const insertPayload: Record<string, unknown> = {
      ...
      signature_clarification:
        typeof signatureClarificationInput !== 'undefined' ? signatureClarificationInput : null,
      signature_path: signaturePathToPersist ?? null,
      signature_uploaded_at:
        signaturePathToPersist
          ? signatureUploadedAtValue ?? new Date().toISOString()
          : null,
    };
