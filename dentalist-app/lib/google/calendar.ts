...
export async function createCalendar(
  tokens: OAuthTokenSet,
  params: { summary: string; description?: string; timeZone?: string },
) {
  const { calendar, latestCredentials } = await ensureAuthorizedCalendar(tokens);
  const response = await calendar.calendars.insert({
    requestBody: {
      summary: params.summary,
      description: params.description,
      timeZone: normalizeTimeZone(params.timeZone ?? DEFAULT_TIME_ZONE),
    },
  });

  return { calendar: response.data, latestCredentials };
}
